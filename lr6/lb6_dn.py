...
def bot_pass(message):
    global results
    if message.text == 'pass123456': # TODO: id пользователя из белого листа
        cursor.execute(
            """
            SELECT id as Номер,
                    date_start as Время_как_загорелся_зеленый,
...
def check_dostup(start_value, value_bd, mess):
    ...
    if val+1 < len(value_bd):
        bot.send_message(mess.from_user.id, text='Выгрузить еще?', reply_markup=keyboard)
    else:
...
def check_start(start_value, value_bd, msg):
    global start_val
    try:
        num = 1
        ...
        if sms != '':
            bot.send_message(msg.from_user.id, sms)
        ...
        if val+1 < len(value_bd):
            bot.send_message(msg.from_user.id, text='Выгрузить еще?', reply_markup=keyboard)
        else:
            bot.send_message(msg.from_user.id, 'Увы, но список подошел к концу')
            start_val = 0
            bot.send_message(msg.from_user.id, default_message())
        return
